...
if( isset( $_GET[ 'b' ] ) && $_GET[ 'b' ] == 1 )
{
	...
	if( !empty( $_GET[ 'cpcff_addons_active_list' ] ) && is_array( $_GET[ 'cpcff_addons_active_list' ] ) )
	{
		...
		foreach( $_GET[ 'cpcff_addons_active_list' ] as $addon )
		{
			if( isset( $cpcff_addons_objs_list[ $addon ] ) )
			{
				$cpcff_addons_objs_list[ $addon ]->activate();
			}
		}
	}
...
<script type="text/javascript">
 function cp_activateAddons()
 {
	...
	document.location = 'admin.php?page=cp_calculated_fields_form&b=1&r='+Math.random()+( ( cpcff_addons_active_list.length ) ? '&'+cpcff_addons_active_list.join( '&' ) : '' )+'&_cpcff_nonce=<?php echo wp_create_nonce( 'session_id_'.CP_SESSION::session_id() ); ?>#addons-section';
 }
 ...
 function cp_updateConfig()
 {
    if (confirm('<?php _e( 'Are you sure that you want to update these settings?', 'calculated-fields-form' ); ?>'))
    {
        ...
        document.location = 'admin.php?page=cp_calculated_fields_form&ecr='+ecr+'&ac=st&scr='+scr+'&chs='+chs+'&jsc='+jsc+'&fmc='+fmc+'&ehr='+ehr+'&hp='+encodeURIComponent( hp )+'&r='+Math.random()+'&_cpcff_nonce=<?php echo wp_create_nonce( 'session_id_'.CP_SESSION::session_id() ); ?>';
    }
 }
