...
class OTWBM_Widget extends WP_Widget {
	...
	function __construct(){
		...
		parent::__construct(
			...
			array(
				'description' => __('With the help of this widget you can add your created Widget Lists to the sidebars', 'otw_bm')
			)
		);
	...
	function form ( $instance ) {
		...
		$htmlForm  = '<p>';
		$htmlForm .= '<label for="'.$field_title.'">'. __('Title:', 'otw_bm') .'</label><br>';
		$htmlForm .= '<input type="text" id="'.$field_title.'" name="'.$field_titleName.'" value="'.$currentWidgetTitle.'" class="widefat"><br><br>';

		$htmlForm .= '<label for="'.$field_id.'">'. __('OTW Blog List Widget:', 'otw_bm') .'</label><br>';
		$htmlForm .= '<select id="'.$field_id.'" name="'.$field_idName.'">';
		$htmlForm .= '<option value="0"> ---'.__('Select Widget', 'otw_bm').'--- </option>';
	...
	function widget ( $args, $instance ) {
		...
		if( !empty( $widgetID ) ) {
	      ...
	      if( file_exists(SKIN_BM_PATH . DS . 'otw-bm-list-'.$widgetID.'-custom.css') ) {
