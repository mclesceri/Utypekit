...
class OTWBMVCExtendAddonClass{
	...
	public function renderShortcode( $params ){
		...
		if( isset( $params['otw_blog_list'] ) && intval( $params['otw_blog_list'] ) ){
			return do_shortcode( '[otw-bm-list id="'.$params['otw_blog_list'].'"]' );
		}
	}
	...
	public function integrateWithVC(){
		...
		if( $this->has_vc ){
			...
			$options = array( __( '--select list--', 'otw_bm' ) => '' );
			...
			vc_map( array(
				"name" => __("Blog Manager", 'otw_bm'),
				"description" => __("Select a post list created with Blog Manager plugin", 'otw_bm'),
				"base" => "otw_bm_vc",
				"class" => "",
				"controls" => "full",
				...
				"category" => __('Blog Manager', 'otw_bm'),
				"params" => array(
					array(
						'type' => 'dropdown',
						'holder' => 'div',
						'class' => '',
						'heading' => __( 'Blog list', 'otw_bm'),
						'param_name' => 'otw_blog_list',
						'value' => $options,
						'description' => __( 'Description for blog list.', 'otw_bm')
					)
				)
			) );
		}
