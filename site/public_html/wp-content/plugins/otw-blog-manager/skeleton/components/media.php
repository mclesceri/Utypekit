...
<?php
  ...
  if( isset( $this->listOptions['image_link'] ) && strlen( $this->listOptions['image_link'] ) ){
	$imageLinkClass = ' otw-link-'.$this->listOptions['image_link'];
  }
?>
...
switch( $postMetaData['media_type'] ) {
  ...
  case 'slider':
    ...
    if( !in_array( $this->listOptions['template'], $mainSlider ) ) {
      ...
      foreach( $sliderImages as $sliderImage ):
        ...
        $media_item .= '
        <li>
          <img src="'.$this->otwImageCrop->resize( $imagePath['path'], $this->imageWidth, $this->imageHeight, $this->imageCrop, $this->imageWhiteSpaces, $this->imageBackground, $this->imageFormat ).'" alt="" data-item="media">
        </li>';
      endforeach;
      $media_item .= '</ul></div>';
      ...
      if( !empty($imgLink) ) {
        ...
        $media_item = '
        <a href="'. $imgLink .'" class="otw-slider-image otw-media-container'.$imageLinkClass.'" data-width="'.$this->imageWidth.'">
          <img src="'.$this->otwImageCrop->resize( $imagePath['path'], $this->imageWidth, $this->imageHeight, $this->imageCrop, $this->imageWhiteSpaces, $this->imageBackground, $this->imageFormat ).'" alt="">
          '.$iconHover.'
        </a>';
        ...
        $media_item = '
          <span class="otw-media-container">
            <img src="'.$this->otwImageCrop->resize( $imagePath['path'], $this->imageWidth, $this->imageHeight, $this->imageCrop, $this->imageWhiteSpaces, $this->imageBackground, $this->imageFormat ).'" alt="" data-item="media" data-width="'.$this->imageWidth.'">
            '.$iconHover.'
          </span>
        ';
  ...
  case 'img':
    ...
    if( !empty($imgLink) ) {
      $media_item = '
      <a href="'. $imgLink .'" class="otw-media-container'. $imageLinkClass.'" data-width="'.$this->imageWidth.'">
        <img src="'.$this->otwImageCrop->resize( $imagePath['path'], $this->imageWidth, $this->imageHeight, $this->imageCrop, $this->imageWhiteSpaces, $this->imageBackground, $this->imageFormat ).'" alt="">
        '.$iconHover.'
      </a>';
    } else {
...
echo $media_item;
?>
  <?php if( isset( $this->listOptions['show-post-icon'] ) && $this->listOptions['show-post-icon'] ) : ?>
  <!-- Blog Type -->
