...
switch($action) {
	case 'customers_list':
		...
		for($p=0;$p<count($people);$p++){
			...
			$list[$p]['Last Name']= stripslashes(urldecode($people[$p]->first_name)).' '.stripslashes(urldecode($people[$p]->last_name));
			$list[$p]['Email'] = $people[$p]->email;
			$level = '';
    ...
    case 'order_people_list':
        ...
        foreach($res AS $p) {
            $list[$o]['ID'] = $p->id;
            $list[$o]['Last Name'] = stripslashes(urldecode($p->first_name)).' '.stripslashes(urldecode($p->last_name));
            $list[$o]['Email'] = $p->email;
	...
	case 'recipe_list':
		...
		for($o=0;$o<count($recipes);$o++) {
			...
			$list[$o]['Title'] = stripslashes(urldecode($recipes[$o]->title));
	   ...
	   for($p=0;$p<count($list);$p++) {
        ...
        <tr>';
            ...
            foreach($list[$p] as $e=>$v) {
                $body .= '
            <td class="listItem">'.stripslashes(urldecode($v)).'</td>';
            }
       }
	   // Draw the close
	   $body .= '
...
function drawPerson($person) {
	$body .= '<div style="display: block; font-weight: bold; font-size: 14pt; width: 100%; padding: 4px">'.stripslashes(urldecode($person->first_name)).' '.stripslashes(urldecode($person->last_name)).'</div>';
	$body .= '<div style="display: block; font-size: 12pt; width: 100%; padding: 4px">'.$person->address1.'</div>';
	if($person->address2) {
		$body .= '<div style="display: block; font-size: 12pt; width: 100%; padding: 4px">'.$person->address2.'</div>';
	}
	$body .= '<div style="display: block; font-size: 12pt; width: 100%; padding: 4px">'.$person->city.', '.$person->state.' '.$person->zip.'</div>';
	if($person->email) {
		$body .= '<div style="display: block; font-size: 12pt; width: 100%; padding: 4px">Email: '.$person->email.'</div>';
	}
	if($person->phone) {
		$body .= '<div style="display: block; font-size: 12pt; width: 100%; padding: 4px">Phone: '.$person->phone.'</div>';
	}
	return( $body );
}
